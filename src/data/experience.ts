...
export const experiences: Experience[] = [
  ...
  // },
  {
    ...
    bullets: [
      "Engineered an AI agent platform using the Claude Agent SDK to automate financial fee calculations, integrating Excel ingestion and tool-calling workflows to return validated results",
      "Re-architected a legacy rule-based system into a modular, config-driven AI agent framework, replacing hardcoded logic with reusable components and dynamic workflow discovery",
      "Standardized deployment by migrating dependency management to uv, a lockfile-based package manager, improving build reproducibility and reducing environment drift",
    ],
    tech: [],
  },
  {
    ...
    bullets: [
      "Engineered an agentic support chatbot with Vercel AI SDK using SSE streaming, tool-calling RAG over internal docs, and prompt guardrails against hallucination and legal advice, reducing support tickets by 40%",
      "Built a multilingual patent machine translation (MT) pipeline over 19M+ BigQuery records, running BERTalign alignment on EC2 GPU compute with automated quality gates to produce a validated parallel corpus",
      "Architected a LangChain-orchestrated agent pipeline generating court-ready DOCX IPR petitions through evaluator-optimizer loops, routing failed citation checks back to the LLM as structured retry feedback",
    ],
    tech: [
      "Python",
      "Flask",
